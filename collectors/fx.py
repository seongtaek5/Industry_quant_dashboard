...
def _append_to_parquet(new_df: pd.DataFrame, parquet_path: Path) -> None:
    ...
    if new_df.empty:
        logger.warning("저장할 신규 환율 데이터가 없습니다.")
        return
...
def collect_fx() -> None:
    ...
    items: list[dict[str, Any]] = config.get("fx", [])
    history_period = config.get("history_period", "2y")
    if not items:
        logger.warning("fx 설정이 비어 있습니다.")
        return
    ...
    parquet_path = Path(__file__).parent.parent / "data" / "fx.parquet"
    try:
        _append_to_parquet(merged, parquet_path)
        logger.info("환율 저장 완료: %s", parquet_path)
    except Exception as e:
        logger.exception("환율 저장 실패: %s", e)
