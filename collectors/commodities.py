...
def _append_to_parquet(new_df: pd.DataFrame, parquet_path: Path) -> None:
    ...
    if new_df.empty:
        logger.warning("저장할 신규 원자재 데이터가 없습니다.")
        return
...
def collect_commodities() -> None:
    """원자재 데이터를 수집해 Parquet로 저장한다."""
    try:
        config = _load_config()
    ...
    items: list[dict[str, Any]] = config.get("commodities", [])
    history_period = config.get("history_period", "2y")
    if not items:
        logger.warning("commodities 설정이 비어 있습니다.")
        return
    ...
    if not frames:
        logger.warning("수집된 원자재 데이터가 없습니다.")
        return

    merged = pd.concat(frames, axis=1, join="outer").sort_index()
    merged = merged[~merged.index.duplicated(keep="last")]

    parquet_path = Path(__file__).parent.parent / "data" / "commodities.parquet"
    try:
        _append_to_parquet(merged, parquet_path)
        logger.info("원자재 저장 완료: %s", parquet_path)
    except Exception as e:
        logger.exception("원자재 저장 실패: %s", e)
